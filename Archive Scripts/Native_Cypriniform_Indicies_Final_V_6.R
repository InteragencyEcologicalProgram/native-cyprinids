...
Sample_Table_Final <- 
  ...
  filter(MethodCode == "SEIN",
         ...
         GearConditionCode == "1") # Only using seines with condition code 1
head(Sample_Table_Final)
str(Sample_Table_Final)

# Outputting final sample data
write.csv(Sample_Table_Final, 
          "Data/Sample_Table_Final.csv")


# Setting min and max sample ids to optimize catch query
sample_start <- min(Sample_Table_Final$SampleID)
sample_end <- max(Sample_Table_Final$SampleID)
...
Catch_Query <- gsub("[\r\n]", " ",
                    sprintf(
                      "SELECT * FROM Catch WHERE SampleID BETWEEN '%s' AND '%s';",
                      sample_start,
                      sample_end))

Catch_Table <- DBI::dbGetQuery(conn = DJFMP_Database_Con, 
                               statement = Catch_Query)
head(Catch_Table)
str(Catch_Table)

write.csv(Catch_Table, 
          "Data/Catch_Table.csv")
...
SASU_Index_Dec_Final$Delta_Wide_Index = rowSums(SASU_Index_Dec_Final[,11:13])
...
# Delta = OUT *Changing to SJR + SAC based on Brain's suggestion
...
DELTA_Mean_Discharge_SAPM <- 
  ...
  filter(Mo %in% c(4:5),
         Year %in% c(1995:2019)) %>% 
  group_by(Year) %>% 
  summarise(Mean_DELTA_Discharge = mean(SJR + SAC))
DELTA_Mean_Discharge_SAPM
...
Timing_Flow_DELTA <- 
  ...
  filter(Mo %in% c(1:5),
         Year %in% c(1995:2019)) %>% 
  group_by(Year) %>% 
  summarise(DELTA_Flow_Timing = sum(Julian_Date * (SJR + SAC))/sum(SJR + SAC))
...
Timing_Flow_Water_Year_DELTA <- 
  ...
  filter(Mo %in% c(10:12, 1:5),
         Water_Year %in% c(1995:2019)) %>% 
  group_by(Water_Year) %>% 
  summarise(DELTA_Flow_Timing_WY = sum(Julian_Date_Water_Year * (SJR + SAC))/sum(SJR + SAC))
...
FP_Inudation <- 
  ...
  filter(Mo %in% c(1:6),
         Year %in% c(1995:2019)) %>% 
  group_by(Year) %>% 
  summarise(FP_Inudation = sum(YOLO >= 4000))
head(FP_Inudation)
tail(FP_Inudation)
...
SAPM_Model_Data <- 
  as.data.frame(cbind(SAPM_Index_Final[,11:14], 
                      SAPM_Covariates_Scaled))
...
# Creating new variable names to get PCs that load on the same variables
# e.g.,  PC1 = mean discharge, PC2 = discharge timing, etc
SAPM_Model_Data_SAC <- 
  SAPM_Model_Data %>% 
  mutate(A_Mean_SAC_Discharge = Mean_SAC_Discharge,
         B_SAC_Flow_Timing = SAC_Flow_Timing,
         C_SAC_Mean_WT = SAC_Mean_WT) %>% 
  select(A_Mean_SAC_Discharge, B_SAC_Flow_Timing, C_SAC_Mean_WT)
...
SAPM_Model_Data_DELTA <- 
  SAPM_Model_Data %>% 
  mutate(A_Mean_DELTA_Discharge = Mean_DELTA_Discharge ,
         B_DELTA_Flow_Timing = DELTA_Flow_Timing,
         C_DELTA_Mean_WT = DELTA_Mean_WT) %>% 
  select(A_Mean_DELTA_Discharge, B_DELTA_Flow_Timing, C_DELTA_Mean_WT)
...
# Creating new variable names to get PCs that load on the same variables
# e.g.,  PC1 = mean discharge, PC2 = discharge timing, etc
# This did not work...order must be due to something else than ABC order

SPLT_Model_Data_SJ <- 
  SPLT_Model_Data %>% 
  mutate(A_Mean_SJ_Discharge = Mean_SJ_Discharge,
         B_SJ_Flow_Timing = SJ_Flow_Timing,
         C_SJ_Mean_WT = SJ_Mean_WT) %>% 
  select(A_Mean_SJ_Discharge, B_SJ_Flow_Timing, C_SJ_Mean_WT)
